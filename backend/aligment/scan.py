...
class DocScanner(object):
    ...
    def get_contour(self, rescaled_image, output_path=None):
        ...
        IM_HEIGHT = rescaled_image.shape[0]
        IM_WIDTH = rescaled_image.shape[1]
        ...
        if not approx_contours:
            ...
            #rint(approx_contours)
            screenCnt = max(approx_contours, key=cv2.contourArea)
            #print('======')
            #print(screenCnt)
            
        return screenCnt.reshape(4, 2), edged

    def scan(self, ori, image, output_path=None, binary=False):

        RESCALED_HEIGHT = 512.0
        ...
        ratio = ori.shape[0] / RESCALED_HEIGHT
        # orig = image_pad.copy()
        # rescaled_image = imutils.resize(image, height = int(RESCALED_HEIGHT))
        # rescaled_image_pad = imutils.resize(image_pad, height = int(RESCALED_HEIGHT))


        # get the contour of the document
        img_point = ori.copy()
        screenCnt, edges = self.get_contour(image, output_path)
        # for point in screenCnt:
        #     cv2.circle(img_point, (int(point[0] * ratio), int(point[1] * ratio)), 15, (0,0,255))
        # ```print(screenCnt.shape)

        # apply the perspective transformation
        
        warped = transform.four_point_transform(ori, screenCnt * ratio)
